    ...
    if chat_db:
        ...
        if chat_member.restricted_by:
            ...
            if (not client.get_chat_member(chat_id, (client.get_me()).id).status == "administrator"
                ...
                client.send_message(
                    chat_id,
                    f"❗ **{cb.from_user.mention} sedang mencoba untuk mengaktifkan Suaranya sendiri tetapi saya tidak dapat mengaktifkannya karena saya bukan admin dalam obrolan ini, tambahkan saya sebagai admin lagi.**\n__#Meninggalkan obrolan ini...__",
                )

            else:
                client.answer_callback_query(
                    cb.id,
                    text="❗ Peringatan! Jangan tekan tombol saat Anda tidak dapat berbicara.",
                    show_alert=True,
                )
            ...
            try:
                ...
                try:
                    sent_message = message.reply_text(
                        "Welcome {} 🙏 \n **Anda belum bergabung dengan Saluran @{} kami**👷 \n \nSilakan Bergabunglah dengan [Saluran Kami](https://t.me/{}) dan tekan tombol **BUKA SAYA**. \n \n ".format(
        ...
        if len(message.command) > 1:
            ...
            if input_str.lower() in ("off", "no", "disable"):
                ...
                try:
                    ...
                    message.reply_text(
                        f"✅ **Paksa Berlangganan Diaktifkan**\n__Paksa Berlangganan diaktifkan, semua anggota grup harus berlangganan [saluran] ini(https://t.me/{input_str}) untuk mengirim pesan di grup ini.__",
                        disable_web_page_preview=True,
                    )
                except UserNotParticipant:
                    message.reply_text(
                        f"😕 **Bukan Admin di Channel**\n__Saya bukan admin di [Saluran](https://t.me/{input_str}).Tambahkan saya sebagai admin untuk mengaktifkan ForceSubscribe.__",
                        disable_web_page_preview=True,
                    )
            ...
            if sql.fs_settings(chat_id):
                message.reply_text(
                    f"✅ **Paksa Berlangganan diaktifkan di obrolan ini.**\n__Untuk [Saluran] ini(https://t.me/{sql.fs_settings(chat_id).channel})__",
                    disable_web_page_preview=True,
                )
            else:
...
__help__ = """
*Paksa Berlangganan:*
❏ Presetend dapat membisukan anggota yang tidak berlangganan saluran Anda sampai mereka berlangganan
...
❏ Tambahkan saya di saluran Anda sebagai adminn 
...
❏ /fsub disable - Untuk mematikan ForceSubscribe..
   🔑 Jika Anda menonaktifkan fsub, Anda perlu mengatur lagi untuk bekerja.. /fsub {channel username}
